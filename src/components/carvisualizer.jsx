...
function Model(props) {
  ...
  if(currentCar === '/supra.glb'){
    ...
    useEffect(() => {
      materials['Toyota_Supra_2020_with_Interior_noTTo3Ds_carpaint.001'].color.set(props.color);
      materials['Toyota_Supra_2020_with_Interior_noTTo3Ds_leatherRed.001'].color.set(props.color);
    }, [materials, props.color]);
  
    useEffect(() => {
      materials['Toyota_Supra_2020_with_Interior_noTTo3Ds_rimB.001'].color.set(props.wheelColor);
    ...
    useEffect(() => {
      materials['2021_Audi_A5_Sportback_S-Line_Ibis_White'].color.set(props.color);
    },[materials, props.color])

    useEffect(() => {
      materials['2021_Audi_A5_Sportback_S-Line_Rims1'].color.set(props.wheelColor);
    },[materials, props.wheelColor])
    ...
    <>
      <Canvas dpr={[1, 2]} camera={{ fov: 60 }} className="absolute">
        ...
        <Stage environment={'sunset'}>
          <Model car={props.car.object} scale={1} color={props.color} wheelColor={props.wheelColor} rotation={[0, 6, 0]} />
        </Stage>
      </Canvas>
    </>
  );
